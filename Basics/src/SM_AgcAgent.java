...
public class SM_AgcAgent extends AgcAgent{
	...
	protected ArrayList<ArrayList<Integer>> optionsToDecide;
	...
	public SM_AgcAgent(double lamb, char var, int type2,boolean tab,String weight) {
		...
		optionsToDecide = new ArrayList<ArrayList <Integer>> ();
...
//				System.out.println();
	...
	protected void vote_preference(){
		if (localView.size()>0)	{
			for (int i=0;i<myAgents.size();i++){
...
//		System.out.println("Agent: " + this.getIdAgent() + ". MY VALUE: " + this.getValue());
		ArrayList <Integer> optionsPoints =  new ArrayList <Integer> ();
		socialGain = 0;
		preferences.clear();
		flagsCatcher.clear();
		allPossibles.clear();
...
//		System.out.print("Agent: " + this.getIdAgent() + ". MY NEXT: ");
		for (int k=0;k<Starter.getNumOfVariables();k++){
			if(preferences.get(k)>0){
				optionsPoints.add((int) (preferences.get(k)*(myAgents.size()+1)));
			}
			else{
			optionsPoints.add(0);
			}
//			System.out.print(optionsPoints.get(k)+", ");	
		}
		optionsToDecide.add(optionsPoints);
//		System.out.println();
...
/**/		}
	...
	/*
	 * creating the hashtables according the number of neighbors
	 */
